...
- يمكن تعديل `ADMIN_ID` أو `CHAT_ID` داخل `.env` لاحقاً لتحديد من يمكنه رؤية الإحصاءات والمعلومات وجميع بيانات المستخدمين.

### ميزات المشرف

المستخدم الذي يُدرج في `ADMIN_ID(S)` سيحصل على صلاحيات إضافية عبر زر **⚙️ إدارة** في القائمة الرئيسية، مثل:

- 📣 بث رسالة إلى جميع المستخدمين الذين بدأوا المحادثة (حتى لو لم يسجلوا).
- ⛔ حظر أو ✅ رفع حظر عن مستخدم (بواسطة معرفه).
- 📋 عرض قائمة المستخدمين الذين بدأوا المحادثة مع أسمائهم.

بعد اختيار أحد هذه الخيارات، سيُطلب منك إدخال النص أو المعرف المطلوب في المحادثة.

المستخدمون العاديون لا يمكنهم الوصول إلى هذه الأدوات، وعند الضغط على أزرار الإحصائيات/سجل سيعرضون فقط بياناتهم الشخصية (عدد تسجيلاتهم وآخرها).
